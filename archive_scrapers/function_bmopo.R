...
bmopo_go_thr_archive <- function(startdate){

  paste0("https://www.morgenpost.de/suche/?q=*&sort=neu&p=1") %>%
  ...
  i <- 2
  while (n>0) {
    paste0("https://www.morgenpost.de/suche/?q=*&sort=neu&p=", i) %>%
      purrr::map_df(~bmopo_get_url(.)) %>% 
...
bmopo_go_thr_archive("2023-01-31") -> valid_links
