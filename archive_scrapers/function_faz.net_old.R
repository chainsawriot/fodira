...
faz_get_titles <- function(html){
  
  #html <- remDr$getPageSource()[[1]]
  #html <- pjs_session$getSource()
  
  rvest::read_html(html) %>% 
    rvest::html_elements(xpath = "//div[contains(@class, 'hitlist ')]/div[contains(@class, 'article-row clearfix')]/h2[1]") %>% 
    rvest::html_text(trim = TRUE) -> item_title
  
  rvest::read_html(html) %>% 
    rvest::html_elements(xpath = "//div[contains(@class, 'hitlist ')]/div[contains(@class, 'article-row clearfix')]//h3") %>% 
    rvest::html_text(trim = TRUE) -> item_title2
  
  df <- data.frame(item_title, item_title2)
  return(df)
}
...
faz_get_urls <- function(title){
  remDr$navigate(paste0("https://duckduckgo.com/?q=", title, "+site%3Afaz.net"))
  print(remDr$getCurrentUrl())
  ...
  item_link <- item_link[stringr::str_which(item_link, "faz.net")]
