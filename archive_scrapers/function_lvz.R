...
lvz_get_url <- function(url){
  ...
  pjs_session$getSource() %>% lvz_get_links() -> df
  return(df)
}

lvz_go_thr_archive <- function(startdate, enddate){
  ...
  V1 %>%
    paste0("https://www.lvz.de/archiv/artikel", .) %>%
    purrr::map_df(~lvz_get_url(.)) -> valid_links
...
lvz_go_thr_archive("2022-01-01", "2022-05-01") -> valid_links1

lvz_go_thr_archive("2022-05-02", Sys.Date()) -> valid_links2

valid_links <- dplyr::distinct(rbind(valid_links1, valid_links2))

valid_links %>% dplyr::rename(title = item_title, link = item_link, pubdate = item_pubdate) %>% 
  dplyr::mutate(pub = "LVZ", description = NA) %>%
  dplyr::select(pub, link, pubdate, title, description) -> valid_links

valid_links <- dplyr::distinct(valid_links)

saveRDS(valid_links, "LVZ.RDS")
